...
describe('Branded Types', () => {
  describe('OpenAPIVersion', () => {
    test('validates correct OpenAPI version formats', () => {
      // Test valid formats
      const validVersions = ['3.0.0', '3.0.1', '3.1.0', '3.2', '3.0', '3.1'];
    ...
    test('rejects invalid OpenAPI version formats', () => {
      ...
      const invalidVersions = ['2.0.0', '4.0.0', 'abc', '', '3', '3.0.0.0'];

      invalidVersions.forEach((version) => {
        expect(() => createOpenAPIVersion(version)).toThrow();
        expect(() => createOpenAPIVersion(version)).toThrow(
          `Invalid OpenAPI version format: ${version}`
        );
      });
    });
  ...
  describe('PathParam', () => {
    test('creates path parameter branded type', () => {
      ...
      const params = ['{id}', '{petId}', '{userId}', 'simple'];
